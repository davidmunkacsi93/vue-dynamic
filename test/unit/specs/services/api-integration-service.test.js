...
describe('Tests for api-integration-service integrateNewApi function:', () => {
  ...
  it('must throw an error, when swagger parsing fails.', async () => {
    spyOn(SwaggerParser, 'validate').and.returnValue(
      Promise.resolve({ swagger: '2.0.' })
    );

    spyOn(SwaggerParser, 'parse').and.callFake(() => {
      throw new Error('Parsing failed');
    });

    ApiIntegrationService.integrateNewAPI('URL').catch((reason) => {
      expect(reason.message).toBe('Error: Parsing failed');
    });
  });
